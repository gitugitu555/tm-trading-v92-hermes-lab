...
import scripts.v92_c_exhaustion_enriched_signal_regime_decay_diagnostic as diag
...
def test_bucket_functions_cover_specified_boundaries() -> None:
    ...
    assert diag._bucket_prior_path(-25.0) == "negative"
    assert diag._bucket_prior_path(0.0) == "flat"
    assert diag._bucket_prior_path(25.0) == "positive"

    assert diag._bucket_cvd(-1.0) == "negative"
    assert diag._bucket_cvd(0.0) == "neutral"
    assert diag._bucket_cvd(1.0) == "positive"

    assert diag._bucket_session(pd.Timestamp("2024-01-01 02:00:00")) == "asia"
    assert diag._bucket_session(pd.Timestamp("2024-01-01 09:00:00")) == "europe"
    assert diag._bucket_session(pd.Timestamp("2024-01-01 14:30:00")) == "overlap"
    assert diag._bucket_session(pd.Timestamp("2024-01-01 18:00:00")) == "us"
...
def test_build_report_smoke(monkeypatch) -> None:
    ...
    assert "V9.2 Hermes C Exhaustion Enriched Signal Regime Decay Diagnostic" in report
