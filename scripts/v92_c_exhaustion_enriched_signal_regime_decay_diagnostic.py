#!/usr/bin/env python3
"""Aggregate-only enriched decay diagnostic for the V9.2 C_Exhaustion branch."""
...
DEFAULT_OUTPUT = Path("reports/hermes_runs/v92_HERMES_C_EXHAUSTION_ENRICHED_SIGNAL_REGIME_DECAY_DIAGNOSTIC.md")
...
PRIOR_PATH_BUCKET_ORDER = ["negative", "flat", "positive", "n/a"]
CVD_BUCKET_ORDER = ["negative", "neutral", "positive", "n/a"]
SESSION_BUCKET_ORDER = ["asia", "europe", "overlap", "us", "session_unknown"]
VOLATILITY_BUCKET_ORDER = ["<25", "25-50", "50-100", ">100", "n/a"]
RANGE_TREND_BUCKET_ORDER = ["trend_continuation", "failed_reversal", "range_expansion", "range", "mixed", "n/a"]
...
def _build_context(trades: pd.DataFrame, bars: pl.DataFrame) -> pd.DataFrame:
    ...
    bars_pd["high"] = pd.to_numeric(bars_pd["high"], errors="coerce")
    bars_pd["low"] = pd.to_numeric(bars_pd["low"], errors="coerce")
...
def _interpretation_label(frame: pd.DataFrame, bucket_tables: dict[str, list[dict[str, object]]], interaction_tables: dict[str, list[dict[str, object]]]) -> str:
    ...
    if best_score <= 25.0:
        return "mixed_context_degradation_no_single_driver"
    if best_field == "volatility_label":
        return "volatility_context_degradation_dominant"
    if best_field == "range_trend_label":
        return "range_trend_context_degradation_dominant"
    ...
    if best_field == "prior_bar_return_path":
        return "prior_path_context_degradation_dominant"
...
def build_report(trade_log_path: Path, bar_dir: Path) -> tuple[str, dict[str, object]]:
    ...
    for field in [
        ...
        bucket_field = field
        if field == "distance_from_vwap_bucket":
            bucket_field = "distance_from_vwap_bucket"
        elif field == "distance_from_recent_high_low_bucket":
            bucket_field = "distance_from_recent_high_low_bucket"
        elif field == "prior_bar_return_path_bucket":
            bucket_field = "prior_bar_return_path_bucket"
        elif field == "cvd_delta_bucket":
            bucket_field = "cvd_delta_bucket"
    ...
    if explanation == "volatility_context_degradation_dominant":
        explanation_reason = "Recent expectancy varied most cleanly by volatility bucket, with the best bucket materially less negative than the aggregate recent sample."
    elif explanation == "range_trend_context_degradation_dominant":
        explanation_reason = "Range/trend buckets separated the sample more clearly than the other safe context fields."
        ...
        explanation_reason = "Distance-from-recent-range buckets provided the clearest separation."
    elif explanation == "prior_path_context_degradation_dominant":
        explanation_reason = "Prior-bar return path was the cleanest pre-entry separator."
    ...
    report: list[str] = []
    report.append("# V9.2 Hermes C Exhaustion Enriched Signal Regime Decay Diagnostic")
    report.append("")
    report.append("## Purpose")
    report.append("")
    report.append("- Determine whether recent C Exhaustion mixed degradation can be explained by safe known-at-entry and pre-entry explanatory fields.")
    ...
    report.append("## Single-Field Attribution")
    report.append("")
    for field, rows in bucket_tables.items():
    ...
    if explanation == "inconclusive_due_to_sparse_segments":
        stop_go = "blocked_due_to_missing_required_inputs"
    else:
        improved_segments = [row for rows in bucket_tables.values() for row in rows if row["recent_count"] and row["historical_count"] and row["recent_expectancy_bps"] is not None and row["historical_expectancy_bps"] is not None and row["recent_expectancy_bps"] > row["historical_expectancy_bps"] + 25.0 and row["recent_count"] >= 5 and row["historical_count"] >= 5]
        if any(row["recent_count"] >= 10 and row["historical_count"] >= 10 for row in improved_segments) and explanation != "mixed_context_degradation_no_single_driver":
            stop_go = "proceed_to_preregistered_context_filter_design_only"
        elif explanation == "mixed_context_degradation_no_single_driver":
